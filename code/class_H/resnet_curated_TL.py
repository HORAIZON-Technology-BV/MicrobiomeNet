...
model = ResNet(**params)
model.load_state_dict(torch.load('../../data/raw/curated_metagenomics/resnet_curated.pt'))
print(model)
...
def objective(trial):
    ...
    for name,param in model.named_parameters():
        if ('first_layer' in name) | ('head' in name) | ('last_normalization' in name):
            param.requires_grad = True
        else:
            param.requires_grad = False
...
for temp, test in tqdm(sss1.split(microbiome,y)):
    ...
    X_test = torch.tensor(np.asarray(X_test), dtype=torch.float32, device=device)
    ...
    del X_train, y_train, X_val, y_val, X_test, y_test, model, train_loader, val_loader
    gc.collect()
    torch.cuda.empty_cache()
...
output_path = '../../data/output/class_H_TL'
